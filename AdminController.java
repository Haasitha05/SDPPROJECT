...
import com.jfsd.SDPProject.model.Event;
...
public class AdminController
{
	...
	// Controller to display all events and handle deletion
	...
	public ResponseEntity<byte[]> displayEventImage(@RequestParam int eventId) throws SQLException {
	...
	@GetMapping("displayEventImage_d")
	public ResponseEntity<byte[]> displayEventImage_d(@RequestParam int eventId) {
	    try {
	        // Fetch the event from the database by eventId
	        Event event = adminService.getEventById(eventId); // Assuming you have an EventService to handle DB logic

	        if (event == null || event.getImage() == null) {
	            // If the event is not found or does not have an image
	            return ResponseEntity.notFound().build();
	        }

	        // Get the image from the event (assuming it's stored as a Blob)
	        Blob imageBlob = event.getImage();
	        byte[] imageBytes = imageBlob.getBytes(1, (int) imageBlob.length());

	        // Return the image as a response with the appropriate content type (JPEG for example)
	        return ResponseEntity.ok()
	                .contentType(MediaType.IMAGE_JPEG)  // You can change the media type depending on the image format
	                .body(imageBytes);
	    } catch (SQLException e) {
	        // Handle the case if there's an error while reading the image from the database
	...
	public ModelAndView deleteEvent() {
	    ...
	    // Fetch all events to be displayed on the delete event page
	    List<Event> events = adminService.getAllEvents();  // Assuming your eventService has the method getAllEvents

	    // Add events to the model
	    mv.addObject("events", events);

	    // Set the view name for the delete event page
	    mv.setViewName("deleteEvent");
	    return mv;
	}
	@PostMapping("deleteEvent")
	public String deleteEvent(@RequestParam("eventId") int eventId) {
	    // Call the service method to delete the event by its ID
	...
	public ModelAndView viewAllEvents(@RequestParam(defaultValue = "1") int page) {
	    ModelAndView mv = new ModelAndView();
	    List<Event> events = adminService.getAllEvents(); // Fetch all events from the database
	    mv.addObject("events", events);  // Pass events to the view
	    mv.setViewName("viewAllEvents");  // Ensure this corresponds to your JSP file
	    return mv;
	}
	...
	public ModelAndView insertEvent(HttpServletRequest request, @RequestParam("image") MultipartFile file, HttpSession session) throws IOException, SerialException, SQLException {
	    ...
	    event.setEventDate(java.sql.Date.valueOf(eventDate)); // Convert String to SQL Date
	    event.setLocation(location);
	    event.setImage(blob);

	    // Call service method to handle the insertion
	    String message = adminService.addEvent(event);

	    // Prepare response view
	    ModelAndView mv = new ModelAndView();
	    mv.setViewName("manageEvents");
	    mv.addObject("message", message);
	    return mv;
	...
	public ResponseEntity<?> displayAllEvents(@RequestParam int eventId) throws SQLException {
	    // Validate admin access (Replace with your validation logic)
	    ...
	    // Fetch the event by ID using the admin service
	    Event event = adminService.getEventById(eventId);
	    ...
	    // Return event details if the event exists
